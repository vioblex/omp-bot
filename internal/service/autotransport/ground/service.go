...
type AutotransportGroundService struct {
	list []autotransport.Ground
}

func (s *AutotransportGroundService) initEntities() {
...
func NewAutotransportGroundService() *AutotransportGroundService {
	service := AutotransportGroundService{}
	service.initEntities()

	return &service
}

func (s *AutotransportGroundService) Describe(groundID uint64) (*autotransport.Ground, error) {
...
func (s *AutotransportGroundService) List(cursor uint64, limit uint64) ([]autotransport.Ground, error) {
...
func (s *AutotransportGroundService) Create(ground autotransport.Ground) (uint64, error) {
...
func (s *AutotransportGroundService) Update(groundID uint64, ground autotransport.Ground) error {
	if err := s.checkSliceIndex(groundID); err != nil {
		return err
	}
...
func (s *AutotransportGroundService) Remove(groundID uint64) (bool, error) {
	if err := s.checkSliceIndex(groundID); err != nil {
		return false, err
	}
...
func (s *AutotransportGroundService) Count() uint64 {
	return uint64(len(s.list))
}

func (s *AutotransportGroundService) checkSliceIndex(groundID uint64) error {
	length := s.Count()
	if groundID >= length {
		return errors.New(
